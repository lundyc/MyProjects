...
MySpace._Application.prototype = {
	...
	keyDisabled : function(key) {
		...
		if (!this._maintenanceConfigs)
			this._maintenanceConfigs = Sys.Serialization.JavaScriptSerializer.deserialize(MySpace.ClientMaintenanceConfigs);
      
		if (this._maintenanceConfigs[key.toLowerCase()])
			return this._checkTestRange(MySpace.ClientContext.UserId, this._maintenanceConfigs[key.toLowerCase()]);
		else
			return false;	
	},
	...
	_checkTestRange : function(userRange, config) {
      var disabled = false;
      
      for (var i = 0; i < config.length; i++)
          disabled = disabled || userRange >= config[i].lb && userRange <= config[i].ub;
...
MySpace.UI.HeaderStatusMood.prototype={
	initialize:function(){
		...
		if(this._statusNode.childNodes.length > 0){
			this._currentStatus=this._statusNode.childNodes[0].nodeValue;
			this._currentMood=this._moodNameNode.getAttribute("value");
	...
	_getStatusFromCookie:function(){
		...
		if(!("MoodId" in vals && "MoodName" in vals && "MoodImg" in vals)){return null;}
		
		if(vals.StatusUid != MySpaceClientContext.UserId){return null;}
		
		return {status: vals.Status, moodValue: vals.MoodId, moodName: vals.MoodName, moodPictureURL: vals.MoodImg};
	},
	_saveStatusInCookie:function(vals){
		...
		cookieVals.Status=vals.status;
		cookieVals.StatusUid=MySpaceClientContext.UserId;
		cookieVals.MoodId=vals.moodValue;
		cookieVals.MoodName=vals.moodName;
		cookieVals.MoodImg=vals.moodPictureURL;
	...
	_createUpdateDialog:function(){
		...
		dlg.innerHTML=
			...
			'			<div id="hsmStatusChoices"></div>' +
			'		</td>' +
			'	</tr>' +
			'	<tr>' +
			'		<td class="hsmLabel">' + MySpaceRes.Status.Mood + '</td>' +
			'		<td><select id="hsmMoodSelect" tabindex="501"></td>' +
			'	</tr>' +
			'	<tr>' +
			'		<td></td>' +
			'		<td>' +
			'			<div id="hsmCancelBtn" class="hsmButton" tabindex="503">' +
							MySpaceRes.HomeDisplay.Cancel +
			'				<div class="hsmButtonRight"></div>' +
			'			</div>' +
			'			<div id="hsmDoneBtn" class="hsmButton" tabindex="502">' +
		...
		this._moodSelect=$get("hsmMoodSelect");
		this._statusChoicesNode=$get("hsmStatusChoices");
		...
		this._addHandler(this._statusInput, "focus", "_onFocusStatusInput");
		this._addHandler(this._statusInput, "blur", "_hideStatusChoices");
		this._addHandler(this._statusInput, "keypress", "_hideStatusChoices");
		this._addHandler(this._updateDialog, "keypress", "_enterKeyHandler");
		this._addHandler(document, "keypress", "_escKeyHandler");
		
		this._call("GetLists", {}, "_populateLists", "_onListsError");
	},
	
	_populateLists:function(result){
		for(var moodVal in result.moods){
			var moodName=result.moods[moodVal];
			var optionElement=new Option(moodName, moodVal);
			this._moodSelect.options[this._moodSelect.options.length]=optionElement;
		}
		this._updateMoodSelect();
		
		for(var i=0;i<result.statusChoices.length;i++){
			var choice=result.statusChoices[i];
			var el=document.createElement("a");
			el.href="#";
			el.innerHTML=choice;
			this._addHandler(el, "mousedown", "_onClickStatusChoice");
			this._statusChoicesNode.appendChild(el);
		}
	},
	_onListsError:function(){
		var opt=new Option(this._moodNameNode.childNodes[0].nodeValue, this._currentMood);
		this._moodSelect.options[0]=opt;
	},
	
	_onClickStatusChoice:function(e){
		this._cancelEvent(e);
		this._statusInput.value=e.target.childNodes[0].nodeValue;
		this._hideStatusChoices();
	},
	
	_hideStatusChoices:function(e){
		var self=this;
		setTimeout(function(){
			self._statusChoicesNode.style.display="none";
			self._moodSelect.style.visibility="";
		}, 0);
	},
	
	_updateMoodSelect:function(){
		if(!this._moodSelect.options.length){ return; }
		
		this._moodSelect.value=this._currentMood;
	},
	...
	_onFocusStatusInput:function(e){
		if(this._statusChoicesNode.childNodes.length>0){
			var dlgLoc=Sys.UI.DomElement.getLocation(this._updateDialog);
			var inpLoc=Sys.UI.DomElement.getLocation(this._statusInput);
			var off=(Sys.Browser.agent==Sys.Browser.Safari) ? 0 : -1;
			this._statusChoicesNode.style.top=(inpLoc.y-dlgLoc.y+this._statusInput.offsetHeight+off)+"px";
			this._statusChoicesNode.style.left=(inpLoc.x-dlgLoc.x+off)+"px";
			if(Sys.Browser.agent==Sys.Browser.InternetExplorer && Sys.Browser.version <= 6){
				this._moodSelect.style.visibility="hidden";
			}
			this._statusChoicesNode.style.display="block";
		}
		
		var inp=this._statusInput;
		setTimeout(function(){
			inp.select();
		}, 50);
	},
	...
	_saveStatus:function(e){
		...
		var args={
			...
			mood:this._moodSelect.value || 0
		};
		
		this._doneBtn.childNodes[0].nodeValue=MySpaceRes.Common.PleaseWait;
		
		this._call("SaveMoodStatus", args, "_onSaveSuccess", "_onSaveError");
	...
	_onSaveSuccess:function(result){
		...
		if(!result.status){ result.status=MySpaceRes.Status.none; }
		...
		this._currentMood=result.moodValue;
		this._currentStatus=this._statusNode.childNodes[0].nodeValue;
		this._truncateStatus();
